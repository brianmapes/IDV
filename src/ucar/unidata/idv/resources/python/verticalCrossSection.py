#These are jython procedures that get called by the JythonControl
...
# setJythonComponent sets the GUI
    ...
    #if len(displayControl.getAnimationTimes())==0: #BEM why is this conditional here?
...
#Save state with the setVar/getVar methods on the display
    ...
    legendComp = JLabel("Vertical Cross Section")
...
def exampleProbe (displayControl, probeLocation):
    ...
    # nlev=  sample.getDomainSet().getLengths()[1] was never used so commented out
...
def createMapChart(dataset,displayControl,nlola,levs):
    chart = ChartFactory.createScatterPlot(displayControl.getDataChoice().getName()+" Vertical Cross Section",
